...
config = {
    ...
    'checkpoint_path': './checkpoints/PointNet_6D_600_final/best_model_final_600.pth',
...
def main():
    ...
    with open('./test_results/PointNet_test_results.txt', 'w') as f:
        f.write(f"Accuracy: {final_acc:.6f}\n")
        f.write(f"mIoU: {final_miou:.6f}\n")
    with open('./test_results/PointNet_test_results.csv', 'w', newline='') as f:
        writer = csv.writer(f)
        writer.writerow(['sample', 'accuracy', 'mIoU'])
        writer.writerows(rows)
    print("Results saved to ./test_results/PointNet_test_results.{txt,csv}")
