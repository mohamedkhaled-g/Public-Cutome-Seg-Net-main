...
from dataloader.DataLoader_PNet import ObjDataset
from networks.PointNet import PointNetSegmentation

# General settings matching the paper
# Table 2 (Dental_Springer_BMC_Adjusted_v3): PointNet -> Focal, AdamW, lr 2.4e-5, wd 2.1e-5, batch 1
config = {
    ...
    'save_path': './checkpoints/PointNet_6D_600_final/',
...
def get_args():
    ...
    parser.add_argument('--experiment_name', default='PointNet_Optuna', type=str, help='Experiment name')
...
def main():
    ...
    final_model = PointNetSegmentation(
        num_classes=config['num_classes'],
        in_channels=config['in_channels'],
    ...
    class_weights = dataset.get_label_weights().to(device)
    final_loss_fn = FocalLoss(gamma=final_focal_gamma, alpha=class_weights)
